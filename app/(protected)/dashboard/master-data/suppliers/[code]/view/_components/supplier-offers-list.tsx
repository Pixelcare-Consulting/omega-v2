...
import { getColumns } from "./supplier-offers-table-column"

type SupplierOffersListProps = {
  suppCode: string
  supplierOffers: Awaited<ReturnType<typeof getSupplierOffersBySupplierCode>>
  isLoading: boolean
  callback: () => void
}

export default function SupplierOffersList({ suppCode, supplierOffers, isLoading, callback }: SupplierOffersListProps) {
      ...
      <div className='flex flex-col items-stretch justify-center gap-2 md:flex-row md:items-center md:justify-between'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <DataImportExport
            ...
            onExport={(args) => handleExport({ ...args, data: supplierOffers })}
            code={`supplier-${suppCode}-supplier-offers`}
          />
        </div>
      </div>
    </DataTable>
