...
import { deleteSupplierOffer, getSupplierOffersBySupplierCode, LineItemsJSONData } from "@/actions/supplier-offer"
import { Icons } from "@/components/icons"
import { useDialogStore } from "@/hooks/use-dialog"

type SupplierOfferData = Awaited<ReturnType<typeof getSupplierOffersBySupplierCode>>[number]

export function getColumns(callback?: () => void): ColumnDef<SupplierOfferData>[] {
  return [
    {
      ...
      cell: function ActionCell({ row }) {
        ...
        async function handleDelete() {
          ...
          toast.promise(executeAsync({ id }), {
            loading: "Deleting supplier offer...",
            success: (response) => {
              const result = response?.data

              if (!response || !result) throw { message: "Failed to delete supplier offer!", unExpectedError: true }
