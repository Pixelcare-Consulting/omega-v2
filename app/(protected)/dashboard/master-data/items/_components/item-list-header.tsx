...
import { syncItemMaster } from "@/actions/sap-item-master"

type ItemListHeaderProps = {
  syncMeta: NonNullable<Awaited<ReturnType<typeof getSyncMetaByCode>>>
}

export default function ItemListHeader({ syncMeta }: ItemListHeaderProps) {
  const lastSyncDate = syncMeta?.lastSyncAt || new Date("01/01/2020")

  const { executeAsync, isExecuting } = useAction(syncItemMaster)
