...
import { deleteAccountContact, getAccountById } from "@/actions/account"
...
type AccountContactData = NonNullable<Awaited<ReturnType<typeof getAccountById>>>["contacts"][number]["contact"]

export function getColumns(accountId: string): ColumnDef<AccountContactData>[] {
  return [
    {
      accessorKey: "name",
    ...
    },
    {
      ...
      cell: ({ row }) => <div className='text-xs'>{row.original?.title || ""}</div>,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <span
              className={cn(
                `inline-flex items-center rounded-md px-2 py-1 text-center font-medium ring-1`,
    ...
    },
    {
      ...
      cell: function ActionCell({ row }) {
        ...
        const { executeAsync } = useAction(deleteAccountContact)
        const [showConfirmation, setShowConfirmation] = useState(false)

        const { id, name } = row.original

        async function handleDeleteAccountContact({ accountId, contactId }: { accountId: string; contactId: string }) {
          setShowConfirmation(false)

          toast.promise(executeAsync({ accountId, contactId }), {
            loading: "Removing account contact...",
            success: (response) => {
        ...
        return (
          <>
            ...
            <AlertModal
              ...
              title='Are you sure?'
              description={`Are you sure you want to remove this contact named "${name}" from this account?`}
              onConfirm={() => handleDeleteAccountContact({ contactId: id, accountId })}
