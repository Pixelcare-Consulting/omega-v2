...
import CustomerExcessLineItemForm from "./customer-excess-line-item-form"
import InputField from "@/components/form/input-field"
import ActionTooltipProvider from "@/components/provider/tooltip-provider"
import { Icons } from "@/components/icons"
import { useDialogStore } from "@/hooks/use-dialog"

type CustomerExcessFormProps = {
  isModal?: boolean
  disableCustomerField?: boolean
  customerExcess: Awaited<ReturnType<typeof getCustomerExcessByCode>>
  custCode?: string
  callback?: () => void
}

export default function CustomerExcessForm({ isModal, disableCustomerField, customerExcess, custCode, callback }: CustomerExcessFormProps) {
  ...
  const values = useMemo(() => {
    if (customerExcess) {
      const { customer, lineItems, listOwner, ...data } = customerExcess
  ...
  const form = useForm<CustomerExcessForm>({
    mode: "onChange",
    values,
    resolver: zodResolver(customerExcessFormSchema),
  })

  const { executeAsync, isExecuting } = useAction(upsertCustomerExcess)
  ...
  const customerOptions = useMemo(() => {
    if (!bpMasters) return []
    return bpMasters.map((customer) => ({ label: customer?.CardName || customer.CardCode, value: customer.CardCode, customer }))
  ...
  const onSubmit = async (formData: CustomerExcessForm) => {
    try {
      ...
      if (result?.data && result?.data?.customerExcess && "code" in result?.data?.customerExcess) {
        ...
        setTimeout(() => {
          router.push(`/dashboard/crm/customer-excesses/${result.data.customerExcess.code}`)
        }, 1500)
      }
    } catch (error) {
      console.error(error)
  ...
  const handleCancel = () => {
    ...
    router.push(`/dashboard/crm/customer-excesses`)
  }

  const onFirstLoad = () => {
    getBpMastersExec({ cardType: "C" })
    getUsersExec()
  }
  ...
  //* set customer code if custCode exist
  useEffect(() => {
    if (custCode && customerOptions.length > 0) form.setValue("customerCode", custCode)
  }, [custCode, JSON.stringify(customerOptions)])
        ...
        <form className='grid grid-cols-12 gap-4' onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div className='col-span-12 md:col-span-6 lg:col-span-3'>
            <ComboboxField
              ...
              extendedProps={{ buttonProps: { disabled: disableCustomerField } }}
