...
import { getColumns } from "./requisition-product-availabilities-table-colum"
import { BP_MASTER_SUPPLIER_SCOPE_OPTIONS, BP_MASTER_SUPPLIER_STATUS_OPTIONS } from "@/schema/master-bp"

type RequisitionProductAvailabilityListProps = {
  reqCode: number
  productAvailabilities: Awaited<ReturnType<typeof getProductAvailabilitiesBySupplierCode>>
  isLoading: boolean
}

export default function RequisitionProductAvailabilityList({
  reqCode,
  productAvailabilities,
  isLoading,
}: RequisitionProductAvailabilityListProps) {
      ...
      <div className='flex flex-col items-stretch justify-center gap-2 md:flex-row md:items-center md:justify-between'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <DataImportExport
            ...
            code={`requisition-${reqCode}-product-availabilities`}
          />
        </div>
      </div>
    </DataTable>
