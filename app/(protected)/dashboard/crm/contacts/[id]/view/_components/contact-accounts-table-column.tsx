...
import { deleteContactAccount, getContactById } from "@/actions/contacts"
...
type ContactAccountData = NonNullable<Awaited<ReturnType<typeof getContactById>>>["accountContacts"][number]["account"]

export function getColumns(contactId: string): ColumnDef<ContactAccountData>[] {
  return [
    {
      accessorKey: "name",
    ...
    },
    {
      ...
      cell: function ActionCell({ row }) {
        ...
        const { executeAsync } = useAction(deleteContactAccount)
        const [showConfirmation, setShowConfirmation] = useState(false)

        const { id, name } = row.original

        async function handleDeleteContactAccount({ accountId, contactId }: { accountId: string; contactId: string }) {
        ...
        return (
          <>
            ...
            <AlertModal
              ...
              title='Are you sure?'
              description={`Are you sure you want to remove this account named "${name}" from this contact?`}
              onConfirm={() => handleDeleteContactAccount({ contactId, accountId: id })}
